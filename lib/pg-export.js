...
PGExport.prototype.getExports = function () {
    ...
    function filter(data) {
        return _.chain(data.Contents)
            .filter(function (o) {
                return /(meta.json)$/.test(o.Key) && (new RegExp('^(' + self.key + ')')).test(o.Key);
            })
            .map(function (u) {
                return {
                    path: u.Key
    ...
    var exports = self.list()
        .then(filter)
        .then(function (filtered) {
            return filtered.map(function (u) {
                return self.get({Key: u.path})
                    .then(function (data) {
                        var contents = JSON.parse(data.Body.toString('utf8'));
                        return self.url('getObject', {Key: self.key + '/' + contents.id + '/' + contents.title})
                            .then(function(data) {
                                contents.url = data;
                                return contents;
                            });

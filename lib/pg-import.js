...
PGImport.prototype.importData = function (queries) {
        ...
        return function(chunk) {
            if(!headers) {
                ...
                var columns = headers.map(function(value, index) {
                    var dataType = query.mappings[headers[index]] && query.mappings[headers[index]].type || self.defaultType;
    ...
    queries = queries.filter(function(query){
        var exists = self.pg.querySync("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = '"+query.tablename+"');")[0]['exists']
        if(exists) console.log("Skipping "+query.tablename+" because it already exists");
        return !exists;
    });
